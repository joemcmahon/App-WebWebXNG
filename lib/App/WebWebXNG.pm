...
EOF
  ...
  sub PrintIcons {
      ...
      if ( $Cgi{ar} && $page{Archive} ) {
        ...
        if ($EditAccess) {
          ...
          if ( defined $PageArchive ) {
            my ( $unlocked, $locker ) = $PageArchive->is_unlocked($title);
            IconLink(
              "$ScriptUrl?UnlockFile=true$Back&" . "unlock_target=$title",
    ...
    if ( defined $UserAdmin ) {
      ...
      note(
        "$CurrentUser can" . ( $can_manage ? "" : "not" ) . " manage site" );
      $can_user = (
...
EOF
 ...
 # abort.
  ...
  sub HandleLinks {
    ...
    $EditAccess
      || FatalError(
      "You do not have permission to edit the links on this page");
...
EOF
  ...
  sub HandlePurge {
    ...
    $ModifyAccess
      || FatalError(
      "You do not have permission to remove all former versions.");
...
EOF
  ...
  sub HandleInfo {
    ...
    $ReadAccess
      || FatalError(
      "You do not have permission to see this page's information");
...
EOF
    ...
    if (@mail) {
      ...
      print "</ul><p>\n";
      print "Last notification was on ",
        scalar( localtime( $page{LastMailed} ) );
      print "<p>\n";
    } else {
  ...
  sub HandleProperties {
    ...
    $ModifyAccess
      || FatalError(
      "You do not have permission to edit this page's properties");
...
EOF
      ...
      unless ( defined $good_user ) {
        FatalError(
          "Could not access password file: " . $UserAdmin->unusable() );
      } elsif ( !$good_user ) {
        ReDo( \&UserPWChange, $redo_hash,
